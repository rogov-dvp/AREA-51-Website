DROP TABLE review;
DROP TABLE shipment;
DROP TABLE productinventory;
DROP TABLE warehouse;
DROP TABLE orderproduct;
DROP TABLE incart;
DROP TABLE product;
DROP TABLE category;
DROP TABLE ordersummary;
DROP TABLE paymentmethod;
DROP TABLE customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        NUMERIC(1,0),
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Cute');
INSERT INTO category(categoryName) VALUES ('Disney');
INSERT INTO category(categoryName) VALUES ('Scary');
INSERT INTO category(categoryName) VALUES ('Old School');
INSERT INTO category(categoryName) VALUES ('Happy');
INSERT INTO category(categoryName) VALUES ('Green');
INSERT INTO category(categoryName) VALUES ('HypnoToad Category');


INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Adipose', 1, 'Adipose jumps out from the fat you lose after you take a Fat-reduction drug.',18.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Cyclops',2,'Cyclops only have one eye but they are optimistic about it.',19.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Sully',2,'Sully is current in Monster University having his fun time and reforming the Monster University.',10.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Xenomorph',3,'A very frightening creature with no remorse when it finds you so don't let it find you.',22.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Randall Boggs',2,'Randall has the ability to make himself invisible which allowed him to sneak in some places. He is also a classmate of Sully in Monster University.',21.35);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('ET',4,'Classic alien. Sometimes ET wonders what is special about itself that people come far away to visit him for.',25.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Master Yoda',4,'Most legendary Jedi that has ever lived, a one of a kind. Protect you, will he, Way of the Force, guide he.',99.66);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Darth Vader',4,'He is both a Sith and a Jedi, However, he is not a Master Jedi BUT he does have a seat on the Jedi council. He was once human but is now burned beyond recognition. He would like you to know that "It would be an honor, if you would join him". ',96.96);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('HypnoToad',7,'This toad is nothing too special. Its eyes change shape but even that.. ALL GLORY TO THE HYPNOTOAD', 99999.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Eyebally',5,'He is always happy to cheer you up! He can be hidden in a android. Android comes seperately. *Woosh Woosh Blink Blink* there goes your bling bling', 31.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Green Squidy',5,'He lives happily when he has an aquarium on his head. This lets him breathe NO2 and keep his bad breathe smell to himself ',21.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Baby Yoda',1,'As cute as an alien can get. He will follow you around. The force is strong with this alien.',108.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Paul',5,'His name is Paul, your average Joe alien. Loves to wear brown pants, hold his breathe to go invisible, and would be a star comedian.', 40.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Baby Miners',3,'Be careful of this little adorable things. They have as many teeth as sharks do and will jump on any living thing that is injured. But they are cute, arent they.', 15.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('CIA Alien',6,'This alien is very sneaky and tries to acquire information from you. But he is weak to McDonalds Szechuan Sauce. Give him this and he is all yours', 5.99);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('High Green MJ',6,'He can operate machinery in construction, govern galaxies, and is always done to chillax with you. Great companion!', 19.98);

INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');


DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 91.70)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 18)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 31);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 106.75)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 21.35);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22', 140)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 30);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-17 05:45:11', 327.85)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 3, 4, 10)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 40)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 23.25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 28, 2, 21.05)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 29, 4, 14);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (5, '2019-10-15 10:25:55', 277.40)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 4, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 19, 2, 81)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 20, 3, 10);

-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'images/adipose.png' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'images/Cyclopes.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'images/sully.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'images/randallBoggs.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'images/et.jpg' WHERE ProductId = 5;

-- NEW SQL DDL for lab 10
UPDATE Product SET productImageURL = 'images/Master-yoda.png' WHERE productId = 6;
UPDATE Product SET productImageURL = 'images/DarthVader.png' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'images/GLORY-TO-THE-HYPNOTOAD1.gif' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'images/Eyebally.png' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'images/Green-Squidy.png' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'images/Baby-yoda.png' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'images/Paul.png' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'images/Baby-Miners.png' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'images/CIA-Alien.png' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'images/High-Green-MJ.png' WHERE ProductId = 15;

-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;